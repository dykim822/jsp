...
public class MemberDao {
	// singleton 하나의 객체로 DB에 CRUD처리(create, read, update, delete)
	...
	public int loginChk(String id, String password) {
		...
		try {
			...
			if(rs.next()) {
				...
				result = -1;	// rs.next()로 읽을 값이 없다는 의미(없는 id)
			}
		}catch (Exception e) {
			System.out.println(e.getMessage());
		}finally {
...
//		삭제지만 실질적으로 삭제를 하지 않는다(회사입장에서 데이터를 이용하기 위해)
	...
	public List<Member> list(int startRow, int endRow) {
		...
		String sql = "select * from (select rowNum rn, e.*"
				+ "from (select * from member2 order by reg_date desc) e)"
